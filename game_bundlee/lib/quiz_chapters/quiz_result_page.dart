...
class QuizResultPage extends StatefulWidget {
  ...
  const QuizResultPage(
      {super.key , required this.currentChapter, required this.correctanswer, required this.wronganswer});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          Padding(
            padding: const EdgeInsets.only(top: 230, left: 60, right: 60),
            child: ListTile(
              title: Text("${widget.correctanswer} Doğru",style: const TextStyle(fontSize: 27,fontWeight: FontWeight.w600),),
              contentPadding:
                  const EdgeInsets.only(top: 12, bottom: 12, right: 7, left: 45),
          ...
          Row(
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ListTile(
                  onTap: () {
                    if(widget.currentChapter==1){nextChapter=ChapterTwo();}
                    else if(widget.currentChapter==2){nextChapter=ChapterThree();}
                    else if(widget.currentChapter==3){nextChapter=ChapterFour();}
                    else if(widget.currentChapter==4){nextChapter=ChapterFive();}
                    else if(widget.currentChapter==5){nextChapter=ChapterSix();}
                    else if(widget.currentChapter==6){nextChapter=ChapterSeven();}
                    else if(widget.currentChapter==7){nextChapter=ChapterEight();}
                    else if(widget.currentChapter==8){nextChapter=ChapterNine();}
                    else if(widget.currentChapter==9){nextChapter=QuestionPage();}
